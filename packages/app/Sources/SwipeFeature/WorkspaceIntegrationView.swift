...
struct WorkspaceIntegrationView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            SettingsPageHeader(
                L10n.tr("Keyboard integration"),
                subtitle: L10n.tr("Show the workspace strip when you switch with AeroSpace shortcuts.")
            )
            ScrollView { content }
            HStack { Spacer()
                Button(L10n.tr("Done")) { dismiss() }.keyboardShortcut(.defaultAction)
            }
        }
        .padding(24)
        .frame(width: 620, height: 580)
        .onAppear { refreshHookStatus() }
    ...
    private func reloadAerospace() {
        ...
        Task {
            ...
            do {
                ...
                reloadFailed = false
                reloadMessage = L10n.tr("AeroSpace reloaded")
                refreshHookStatus()
            } catch {
                reloadFailed = true
    ...
    private var content: some View {
        SettingsSection(L10n.tr("Keyboard Switches (Experimental)")) {
    ...
    private var hudOffNote: some View {
        Text(L10n.tr("Turn on “Show workspace names while switching” to see the strip."))
            .font(.system(size: 12))
    ...
    private var actionButtons: some View {
        HStack {
            Button(L10n.tr("Open Config File…")) {
                guard let path = hookStatus?.configPath else { return }
                NSWorkspace.shared.open(URL(fileURLWithPath: path))
            }.disabled(hookStatus?.configPath == nil)
            Button(L10n.tr("Reload AeroSpace config")) {
                reloadAerospace()
            }
            .disabled(isReloading)
            .controlSize(.small)
            Button(L10n.tr("Re-check")) {
                reloadMessage = nil
                refreshHookStatus()
            }
            .controlSize(.small)
    ...
    private var setupSteps: some View {
        VStack(alignment: .leading, spacing: 8) {
            stepRow(1, L10n.tr("Copy the line below — it calls AeroKit whenever the focused workspace changes."))
            stepRow(2, pasteInstruction)
            stepRow(3, L10n.tr("Save the file, then click \"Reload AeroSpace config\" below."))
        }
    }
    ...
    private var pasteInstruction: String {
        ...
        case .needsMerge:
            L10n
                .tr(
                    """
                    Open \(hookStatus?.configPath ?? "~/.aerospace.toml") and replace your \
                    exec-on-workspace-change line with the merged line below.
                    """
                )
        default:
            L10n
                .tr(
                    """
                    Open \(hookStatus?.configPath ?? "~/.aerospace.toml") and add the line below \
                    at the end of the file, \
                    replacing any old AeroKit line.
                    """
                )
        }
    }
    ...
    private var copyButtons: some View {
        HStack(spacing: 8) {
            Button(L10n.tr("Copy config line")) {
                copyToPasteboard(hookDisplayLine, notice: L10n.tr("Config line copied"))
            }
            .disabled(hookDisplayLine == nil)
            Button(L10n.tr("Copy AI prompt")) {
                copyToPasteboard(aiPrompt, notice: L10n.tr("AI prompt copied"))
            }
            ...
            if hookStatus?.wiredPathExists == false, let stale = hookStatus?.wiredPath {
                HStack(spacing: 6) {
                    Image(systemName: "exclamationmark.triangle")
                        .foregroundStyle(.orange)
                    Text(L10n.tr("The hook calls '\(stale)', which doesn't exist — replace it with the line below."))
                        .font(.system(size: 12))
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
                ...
                HStack(spacing: 6) {
                    ...
                    Text(L10n.tr("Configured — AeroSpace reports workspace changes to AeroKit"))
                        .font(.system(size: 12))
                        .foregroundStyle(.secondary)
                }
            }
        case .needsMerge:
            HStack(spacing: 6) {
                Image(systemName: "exclamationmark.circle")
                    .foregroundStyle(.orange)
                Text(L10n.tr("Your exec-on-workspace-change doesn't call AeroKit yet"))
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
            }
        case .absent:
            HStack(spacing: 6) {
                Image(systemName: "xmark.circle")
                    .foregroundStyle(.secondary)
                Text(L10n.tr("Not configured"))
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
            }
        case nil:
            Text(L10n.tr("Checking hook status…"))
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
        }
    }
