...
public final class AppPreferences: ObservableObject {
    ...
    @Published public var hotKey: HotKeySpec {
        didSet { store(hotKey, forKey: Keys.hotKey) }
    }

    @Published public var refreshShortcut: HotKeySpec {
        didSet { store(refreshShortcut, forKey: Keys.refreshShortcut) }
    }

    @Published public var settingsShortcut: HotKeySpec {
        didSet { store(settingsShortcut, forKey: Keys.settingsShortcut) }
    }
    ...
    public init(defaults: UserDefaults = .standard) {
        ...
        hotKey = Self.loadSpec(defaults, Keys.hotKey) ?? .default
        refreshShortcut = Self.loadSpec(defaults, Keys.refreshShortcut) ?? .defaultRefresh
        settingsShortcut = Self.loadSpec(defaults, Keys.settingsShortcut) ?? .defaultSettings
    }

    private static func loadSpec(_ defaults: UserDefaults, _ key: String) -> HotKeySpec? {
        defaults.data(forKey: key)
            .flatMap { try? JSONDecoder().decode(HotKeySpec.self, from: $0) }
    }

    private func store(_ spec: HotKeySpec, forKey key: String) {
