...
public final class WindowImageCapturer: Sendable {
    ...
    /// Window bounds (in points) for every window, fetched in one
    /// WindowServer round trip; drives the per-window capture resolution.
    public func windowBoundsByID() -> [CGWindowID: CGSize] {
        guard let info = CGWindowListCopyWindowInfo([.optionAll], kCGNullWindowID) as? [[String: Any]] else {
            return [:]
        }
        var result = [CGWindowID: CGSize]()
        for entry in info {
            ...
            result[id] = CGSize(width: width, height: height)
        }
        return result
    }

    /// Windows whose point size already exceeds the compose cell capture at
    ...
    /// they stay sharp in the overview; unknown bounds err toward quality.
